...
const int ls_count[16]={
	1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};

void setkeystar(unsigned char bits[],unsigned char K[][48])
{
...
void F(int n,char *ll,char *rr,char *LL,char *RR,unsigned char K[][48])
{
	int i;
...
void encrypt0(unsigned char *text,unsigned char *mtext,unsigned char K[][48])
{
...
void discrypt0(unsigned char *mtext,unsigned char *text,unsigned char K[][48])
{
	char ll[64],rr[64],LL[64],RR[64];
	unsigned char tmp[64];
	int i,j;
...
void des(unsigned char *key,unsigned char *text,unsigned char *mtext)
{
	...
	expand0(key,tmp);
	setkeystar(tmp,K);
	encrypt0(text,mtext,K);
}

void undes(unsigned char *key,unsigned char *text,unsigned char *mtext)
{
	unsigned char tmp[64];
	unsigned char K[17][48];
	expand0(key,tmp);
	setkeystar(tmp,K);
	discrypt0(text,mtext,K);
}
